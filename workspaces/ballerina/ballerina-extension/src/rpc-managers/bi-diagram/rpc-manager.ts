    ...
    async getReadmeContent(): Promise<ReadmeContentResponse> {
        return new Promise(async (resolve) => {
            ...
            if (extension.isWebMode) {
                ...
                const readmeUri = vscode.Uri.joinPath(projectRoot, "README.md");
                try {
                ...
                try {
                    const fileData = await vscode.workspace.fs.readFile(readmeUri);
                    return { content: new TextDecoder().decode(fileData) };
                } catch (err) {
                    console.error("Error reading README.md in web mode:", err);
                    return { content: "" };
                }
            }
            else {
